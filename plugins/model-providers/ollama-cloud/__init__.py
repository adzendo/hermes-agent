...
This profile maps Hermes's ``xhigh`` / legacy ``extra_high`` aliases to
``max`` to unlock DeepSeek V4's "Max thinking" tier through Ollama Cloud.
...
class OllamaCloudProfile(ProviderProfile):
    """Ollama Cloud — maps xhigh/legacy extra_high→max via top-level reasoning_effort."""

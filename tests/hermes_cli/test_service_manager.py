...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    if sys.platform == "darwin":
        # APFS/local macOS tmpdirs can clear the setgid bit on chmod for this
        # unprivileged test fixture; the production s6 layout is Linux-only.
        expected_event_modes.add(0o1730)
    assert event_mode in expected_event_modes, (
        f"event/ mode = {oct(event.stat().st_mode)}, want one of "
        f"{sorted(oct(m) for m in expected_event_modes)}"
    )
...
def test_seed_supervise_skeleton_handles_log_subservice(tmp_path) -> None:
    ...
    expected_log_event_modes = {0o3730}
    if sys.platform == "darwin":
        expected_log_event_modes.add(0o1730)
    assert stat.S_IMODE(log_event.stat().st_mode) in expected_log_event_modes

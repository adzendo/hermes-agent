...
class TestResolveAnthropicToken:
    @pytest.fixture(autouse=True)
    def no_keychain(self, monkeypatch):
        # These resolver tests pin Path.home() to a temp dir.  A real macOS
        # Keychain entry is outside that sandbox and would make the tests
        # depend on the developer machine's Claude Code login state.
        monkeypatch.setattr(
            "agent.anthropic_adapter._read_claude_code_credentials_from_keychain",
            lambda: None,
        )
...
class TestRunOauthSetupToken:
    @pytest.fixture(autouse=True)
    def no_keychain(self, monkeypatch):
        # setup-token tests mock subprocess.run for the interactive CLI call;
        # they should not also probe this machine's real macOS Keychain.
        monkeypatch.setattr(
            "agent.anthropic_adapter._read_claude_code_credentials_from_keychain",
            lambda: None,
        )
